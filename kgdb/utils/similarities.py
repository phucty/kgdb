...
def get_top_k(query, choices, k=20):
    results = process.extract(query, choices, scorer=fuzz.WRatio, limit=k)
    results = [[value, score / 100.0] for value, score, _ in results]
    return results
...
def run_test_closest():
    ...
    for str_1 in str_1_list:
        print(f"\n{str_1:s}")
        for func in funcs:
            c_i, c_s = get_closest(func, str_1)
