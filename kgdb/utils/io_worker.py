...
def read_line_from_file(file_name: str, mode="r"):
    if ".bz2" in file_name:
...
def get_size_obj(num: int, suffix="B"):
    num = abs(num)
    if num == 0:
        return "0B"
    try:
...
def print_status(message: str, is_screen: bool = True, is_log: bool = True):
...
def load_object_csv(file_name, encoding="utf-8"):
    content = []
