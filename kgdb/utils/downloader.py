...
def download_file(
    download_url: str, download_dir: str, replace: bool = False
) -> str | None:
...
def download_files(
    download_urls: List[str], download_dir: str, replace: bool = False
) -> Dict:
    report = {}
...
def print_download_report(download_urls: List[str], report: Dict[str, str]) -> None:
...
def download_dump_dbpedia(download_dir) -> None:
    iw.create_dir(download_dir)

    subprocess.run(
        ["sh", f"{cf.DIR_ROOT}/kgdb/utils/dbpedia_download.sh", download_dir],
